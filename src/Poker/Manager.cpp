...
bool Manager::Move(Poker* poker, istream& in) {
    int orig, dest, num;
#ifdef _CONSOLE
    cout << "--move--" << endl << "input orig, dest, num: ";
    in >> orig >> dest >> num;
    cout << endl;
    cout << "Chose: ";
    poker->printCard(orig, num);
    cout << endl;
    cout << "canMove? ";
#else
    in >> orig >> dest >> num;
    ...
    for (int dest = 0; dest < poker->desk.size(); ++dest) {
        ...
        if (destCards.empty()) {
            ...
            for (int orig = 0; orig < poker->desk.size(); ++orig) {
                ...
                //得到可移动牌数量
                //至少会返回1
                int num = GetPickCount(poker.get(), orig);
                ...
                while (1) {
                    ...
                    if (::CanPick(poker.get(), orig, num)) {//因为Manager也有个CanPick函数，用::访问全局的那个
                        num++;
                    } else {
                        num--;
                        break;
...
bool Manager::DFS(bool& success, int& calc, const string& origTitle, vector<shared_ptr<Action>>& record,
    ...
    //按照评估分大到小排序
    sort(actions.begin(), actions.end(), [](const Node & n1, const Node & n2) {
...
void Manager::Draw(HDC hdc, const RECT& rect) {
    ...
    if (poker && poker->hasGUI) {
        ...
        //顶层牌按z-index从小到大排序
        sort(vecTopCards.begin(), vecTopCards.end(), [](const Card * c1, const Card * c2) {
...
bool Manager::ShowOneHint() {
    ...
    //无动作，或者仅有1个发牌动作
    if (actions.empty() || (actions.size() == 1 && typeid(*actions.front().action) == typeid(ReleaseCorner))) {
