...
int GetPickCount(const Poker* poker, int origIndex) {
    ...
    //从下数第2张牌开始遍历
    //eg. num==4, i=[0,1,2]
    for (int i = poker->desk[origIndex].size() - 2; i >= 0; --i) {
        const Card& card = poker->desk[origIndex][i];
        if (card.suit == suit && point + 1 == card.point) {
            point++;
            num++;
        } else {
            break;
...
bool CanMove(const Poker* poker, int origIndex, int destIndex, int num) {
    ...
    if (destCards.empty())
        return true;
    else if (origTopCard.point + 1 == destCards.back().point) //目标堆叠的最外牌==移动牌顶层+1
        return true;
    return false;
}
...
bool PMove::Do(Poker* inpoker) {
    ...
    if (poker->desk[dest].empty() ||
        ...
        if (poker->hasGUI) {
            //加入点集
            vecStartPt.clear();
            for_each(itOrigBegin, itOrigEnd, [&](const Card & card) {
                vecStartPt.push_back(card.GetPos());
            });
        }
#endif
